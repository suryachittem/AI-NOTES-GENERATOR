...
const History = () => {
  ...
  const openNotes = async (noteId) => {
    ...
    try {
      const res = await axios.get(serverUrl + `/api/v1/notes/${noteId}`, {
        withCredentials: true,
      });
        ...
        <AnimatePresence>
          {isSidebarOpen && (
            <motion.div
              ...
              className="fixed lg:static top-0 left-0 z-50 lg:z-auto w-72 lg:w-auto h-full lg:h-[75vh] lg:col-span-1 bg-black/90 lg:bg-black/80 backdrop-blur-xl border border-white/10 shadow-[0_20px_45px_rgba(0,0,0,0.6)]
